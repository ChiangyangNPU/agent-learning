# agent-learning 多远程仓库同步指南

## 1. 概述

agent-learning 代码同时托管在 **Gitee** 和 **GitHub** 两个远程仓库，本地 `master` 分支分别推送为两个平台的分支名：

| 远程名 | 地址 | 协议 | 推送目标分支 |
|--------|------|------|--------------|
| `gitee` | `https://gitee.com/chiangyangNPU/agent-learning.git` | HTTPS | `master` |
| `github` | `git@github.com:ChiangyangNPU/agent-learning.git` | SSH | `main` |
...
git remote add github git@github.com:ChiangyangNPU/agent-learning.git
# git remote add gitee  https://gitee.com/chiangyangNPU/agent-learning.git   # 克隆自 GitHub 时
...
```bash
# 添加远程（已配置，仅作参考）
git remote add gitee  https://gitee.com/chiangyangNPU/agent-learning.git
git remote add github git@github.com:ChiangyangNPU/agent-learning.git
...
| 远程 | 认证方式 | 说明 |
|------|----------|------|
| `gitee` | HTTPS + 系统凭据管理 | 凭据首次输入后由系统凭据库缓存，无需重复输入 |
...
# 1. 初始提交
git add -A
git commit -m "Initial commit: agent-learning"
...
解决：改用 **SSH** 认证（本机 `id_ed25519` 已注册 GitHub；本仓库 `github` 远程默认即 SSH）：

```bash
git remote set-url github git@github.com:ChiangyangNPU/agent-learning.git
